function ProblemPanel(problem) {
    ...
    function congratulate() {
        ...
        congratsPanel.addClass("conMain");
        
        
        conStateDisplay = $("<div></div>");
	conStateDisplay.addClass("conHead centerText");
	conStateDisplay.append(boldTextElement("Current State"));
	conState = problem.currentState;
	stateCanvas = conState.makeCanvas();
	conStateDisplay.append($(stateCanvas));
	congratsPanel.append(conStateDisplay);
        
        var conMessage = $("<div></div>");
        conMessage.addClass("congrats");
        conMessage.append("Congratulations!");
        congratsPanel.append(conMessage);
        
        var probMessage = $("<div></div>");
        probMessage.addClass("conProb");
        probMessage.append("You solved the problem using");
        congratsPanel.append(probMessage);
        
        var moveNumber = $("<div></div>");
        moveNumber.addClass("conMoveNum");
        moveNumber.append(assistant.moveCount.toString());
        congratsPanel.append(moveNumber);
        
        var moveWord = $("<div></div>");
        moveWord.addClass("conMoves");
        moveWord.append("moves");
        congratsPanel.append(moveWord);
        
        var conButtons = $("<div></div>");
        conButtons.addClass("conButtons");
        
        var continueButton = $("<input></input>");
        
        var quitButton = $("<input></input>");
        
        conButtons.append();
        congratsPanel.append(conButtons);
